...
VSEventSchema = atapi.Schema((
    ...
    atapi.StringField(
        ...
        widget=atapi.StringWidget(
            ...
            description=_(
                u'vs_event_help_bysetpos', 'Comma separated list of '
    ...
    atapi.LinesField(
        ...
        widget=atapi.LinesWidget(
            ...
            description=_(
                u'vs_event_help_exceptions',
                (
                    'Please enter exceptions to recurrence. '
                    'One date per line in format YYYY-MM-DD')
            ),
        ),
    ),
    ...
    atapi.IntegerField(
        ...
        widget=atapi.SelectionWidget(
            label=_(u"vs_event_label_frequency", 'Frequency'),
        )
    ),
    atapi.IntegerField(
        'interval',
        ...
        widget=atapi.IntegerWidget(
            label=_(u"vs_event_label_interval", 'Interval'),
        )
    ),
    atapi.DateTimeField(
        'until',
        ...
        widget=CalendarWidget(
            label=_(u"vs_event_label_repeat_until", "Repeat until"),
            description=_(
                u"vs_event_description_repeat__until",
    ...
    atapi.IntegerField(
        ...
        schemata="recurrence",
        widget=atapi.IntegerWidget(
            label=_(u"vs_event_label_count", 'Count'),
            description=_(
                u"vs_event_description_count",
                "Maxinum number of times the event repeats"),
        )
    ),
...
def modifyEventSchema(schema):
    schema.addField(atapi.BooleanField(
        'wholeDay',
    ...
    schema['startDate'].widget = CalendarWidget(
        label=_(u"label_event_start", "Event Starts"),
        with_time=1,
        with_popup=1,
        js_shortcuts=0,
        ignore_unset_time=1)

    schema['endDate'].widget = CalendarWidget(
        label=_(u"label_event_end", "Event Ends"),
        with_time=1,
        with_popup=1,
        js_shortcuts=0,
        ignore_unset_time=1)
    ...
    schema.addField(
        atapi.DateTimeField(
            ...
            widget=CalendarWidget(
                description=_(
                    u"vs_repeat_events_until_date",
                    "Event repeats until this date"),
                label=_(
                    u"vs_label_repeat_until",
                    "Repeat until"),
                with_time=1)))
    ...
    schema.addField(
        DataGridField(
            ...
            widget=DataGridWidget(
                label=_(
                    u'vs_event_label_roleAttendees',
                    "Attendees and roles"
                ),
                columns={
                    ...
                    'role': SelectColumn(
                        _(u'vs_event_label_roleColumn', "Role"),
                        vocabulary='getAttendeeRoles'
                    ),
                    'show': CheckboxColumn(
                        _(u'vs_event_label_showColumn', "Show")
                    ),
                }),
        ))
...
class VSEvent(ATEvent):
    ...
    def at_post_edit_script(self):
        ...
        if self.getWholeDay():
            self.setStartDate(
                DateTime(self.start().strftime('%Y/%m/%d 00:00:00'))
            )
            self.setEndDate(
                DateTime(self.end().strftime('%Y/%m/%d 23:59:00'))
            )
    ...
    def getAttendeeRoles(self):
        ...
        return atapi.DisplayList(
            (
                (
                    'chair',
                    _(u'vs_event_label_chair', "Chair")
                ),
                (
                    'observer',
                    _(u'vs_event_label_observer', "Observer")
                ),
                (
                    'participant',
                    _(u'vs_event_label_participant', "Participant")
                ),
                (
                    'opt_participant',
                    _(
                        u'vs_event_label_opt_participant',
    ...
    def post_validate(self, REQUEST=None, errors=None):
        ...
        """
        if REQUEST.get('useEndDate', True):
            return super(VSEvent, self).post_validate(
                REQUEST=REQUEST,
                errors=errors
            )
